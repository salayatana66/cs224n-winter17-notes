...
class SoftmaxModel(Model):
  ...
  def add_model(self, input_data):
    ...
    with tf.name_scope("variables"):
      self.W = tf.get_variable("W",shape=(self.config.n_features,
                                           self.config.n_classes),
                                dtype = tf.float32,
                                initializer = tf.zeros_initializer())
      self.b = tf.get_variable("b",shape=(self.config.n_classes,),
                                dtype = tf.float32,
                                initializer = tf.zeros_initializer())
  ...
  def add_loss_op(self, labels,pred):
  ...
  def __init__(self, config):
    ...
    self.loss = self.add_loss_op(self.labels_placeholder,self.pred)
